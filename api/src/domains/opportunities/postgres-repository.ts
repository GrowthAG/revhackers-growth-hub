...
export class PostgresOpportunityRepository {
  ...
  async create(params: CreateOpportunityParams): Promise<OpportunityRecord> {
    ...
    const result = await this.pool.query(
      ...
      [
        params.client_name,
        params.client_email?.toLowerCase() || null,
        params.client_company || null,
        cnpj,
        type,
        leadSource,
        pipelineStage,
        params.diagnostico_id || null,
    ...
    return {
      ...
      opportunity_data: typeof row.opportunity_data === 'string' ? JSON.parse(row.opportunity_data) : row.opportunity_data,
  ...
  async findByCnpj(cnpj: string): Promise<OpportunityRecord | null> {
    ...
    const result = await this.pool.query(
      `SELECT * FROM opportunities WHERE cnpj = $1 AND pipeline_stage != 'lost' ORDER BY updated_at DESC LIMIT 1`,
      [cleanCnpj]
    );
    const row = result.rows[0];
    if (!row) return null;
