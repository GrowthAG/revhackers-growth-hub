...
export interface CompetitorIntelligenceView {
  ...
  spi_category: SPICategory | null;
  ofs_risk_level: OFSRiskLevel | null;
...
export interface CompetitorWithIntelligence {
  ...
  intelligence: CompetitorIntelligenceView | null;
  recent_signals: MarketSignalView[];
}
...
export const intelligenceGcpAdapter = {
  ...
  async listInsights(tenantId: string, projectId: string): Promise<IndustryInsight[]> {
    const res = await intelligenceRequest(`/insights/${projectId}?tenant_id=${tenantId}`);
    const data = (await res.json()) as { data: { insights: IndustryInsight[]; source: string; generated_at: string } };
...
export const lifecycleGcpAdapter = {
  async getContactJourney(contactId: string, tenantId: string): Promise<LifecycleEvent[]> {
    const res = await authenticatedRequest(`/v1/lifecycle/journey/${contactId}?tenant_id=${tenantId}`);
