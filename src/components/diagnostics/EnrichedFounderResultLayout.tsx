...
interface EnrichedFounderResultLayoutProps {
  result: EnrichedFounderResult;
  segmentLabel?: string;
  cnpjInsights?: EnrichedFounderResult['cnpjInsights'];
  onSchedule?: () => void;
  onShare?: () => void;
}
...
const ARCHETYPE_LABEL: Record<EnrichedFounderResult['archetype'], string> = {
  Executor: 'Executor',
...
export const EnrichedFounderResultLayout: React.FC<EnrichedFounderResultLayoutProps> = ({
  result,
  segmentLabel = 'Founders B2B SaaS early-stage',
  cnpjInsights,
  onSchedule,
  onShare,
}) => {
  ...
  const totalLeak = result.dimensions.reduce((acc, d) => acc + d.impactBrl, 0);
    ...
    <div className="w-full bg-white">
        ...
        <div className="max-w-6xl mx-auto px-6 py-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[260px_1fr] gap-6 items-center">
            <div className="flex flex-col items-center justify-center border border-zinc-900 p-6 bg-white">
              ...
              <div className="text-3xl font-black text-zinc-900 leading-none text-center">
                {ARCHETYPE_LABEL[result.archetype]}
              </div>
              <div className="text-xs text-zinc-500 mt-2 font-medium">
                confianca: {Math.round(result.archetypeConfidence * 100)}%
              </div>
              <div className="mt-3 w-full">
                ...
                <div
                  className={cn(
                    'mt-2 inline-block px-3 py-1 text-[10px] font-bold uppercase tracking-wider',
      ...
      <main className="max-w-6xl mx-auto px-6 py-10 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <RevenueLeakCard
            totalAnnualBrl={totalLeak}
            monthlyAverageBrl={Math.round(totalLeak / 12)}
            topDrivers={topDrivers}
          />
        </div>

        {cnpjInsights && <CnpjEnrichmentCard insight={cnpjInsights} />}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="border border-zinc-900 bg-white">
            ...
            <ul className="divide-y divide-zinc-100">
              {result.blindSpots.slice(0, 3).map((s, i) => (
                <li key={i} className="px-6 py-4 text-sm text-zinc-900">
                  {s}
                </li>
              ))}
          ...
          <div className="border border-zinc-900 bg-white">
            ...
            <ul className="divide-y divide-zinc-100">
              {(result.brandingGaps || []).slice(0, 3).map((s, i) => (
                <li key={i} className="px-6 py-4 text-sm text-zinc-900">
                  {s}
                </li>
              ))}
