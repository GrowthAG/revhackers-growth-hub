...
export const ScoreGauge = ({ score, label = "Diagnostic Score", description = "Pontuação Sintética", variant = 'dark' }: ScoreGaugeProps) => {
 ...
 variant === 'dark' ? "bg-white border-zinc-200" : "bg-white border-zinc-200"
 )}>
 <div className="absolute top-6 left-6 text-xs font-sans text-zinc-500 ">
 Index
 </div>
 ...
 <div className={cn(
 "text-2xl md:text-3xl font-semibold leading-none",
 variant === 'dark' ? "text-zinc-900" : "text-black"
 )}>{score}</div>
 <div className="text-xs text-zinc-500 font-sans mt-2 ">Pontos</div>
 </div>
 </div>
 ...
 <p className="text-xs text-zinc-500 max-w-[200px] mx-auto leading-tight font-medium ">
 {description}
 </p>
 </div>
 </div>
